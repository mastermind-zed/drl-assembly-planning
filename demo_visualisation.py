...
parser.add_argument('--ckpt-path', type=str, default='log', help='The folder that contains logs')
...
parser.add_argument('--seed', type=int, default=0, help='the seed of the checkpoint')
...
if __name__ == '__main__':
    ...
    if args.algo == 'dqn' or args.algo == 'ddqn':
        ...
        # Fix: correctly handle the _logs suffix and seed subdirectory
        ckpt_full_path = '{}/{}_logs/env{}/scene{}/seed{}/policy.pth'.format(
            args.ckpt_path, args.algo, args.env_id, args.task_id, args.seed
        )
        print(f"Loading checkpoint from: {ckpt_full_path}")
        policy.load_state_dict(torch.load(ckpt_full_path, map_location=args.device))
    else:

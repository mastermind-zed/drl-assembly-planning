...
class site():
    ...
    def init_component(self):
        ...
        # FRAME
        sco1 = SCO(1, 'beam', x11, y11, 1, x12, y12, 1, 9, 10, 4, 9, 12, 4, 3, [[2,2,1],[3,2,2]], train_on = False)
        self.scos.append(sco1)
        sco2 = SCO(2, 'column', x51, y51, 1, x52, y52, 1, 9, 9, 1, 9, 9, 4, 4, [[0,1,1],[1,1,2]], train_on = False)
        self.scos.append(sco2)
        sco3 = SCO(3, 'column', x61, y61, 1, x62, y62, 1, 9, 13, 1, 9, 13, 4, 4, [[0,1,1],[1,2,2]], train_on = False)
        ...
        sco4 = SCO(4, 'column', x71, y71, 1, x72, y72, 1, 13, 9, 1, 13, 9, 4, 4, [[0, 1, 1], [6, 1, 2]], train_on = False)
        self.scos.append(sco4)
        sco5 = SCO(5, 'column', x81, y81, 1, x82, y82, 1, 13, 13, 1, 13, 13, 4, 4, [[0, 1, 1], [7, 2, 2]], train_on = False)
        self.scos.append(sco5)
        sco6 = SCO(6, 'beam', x21, y21, 1, x22, y22, 1, 10, 9, 4, 12, 9, 4, 3, [[2, 2, 1], [4, 2, 2]], train_on = False)
        self.scos.append(sco6)
        sco7 = SCO(7, 'beam', x31, y31, 1, x32, y32, 1, 13, 10, 4, 13, 12, 4, 3, [[4, 2, 1], [5, 2, 2]], train_on = False)
        self.scos.append(sco7)
        sco8 = SCO(8, 'beam', x41, y41, 1, x42, y42, 1, 10, 13, 4, 12, 13, 4, 3, [[3, 2, 1], [5, 2, 2]], train_on = False)
                    ...
                    if self.sco.type == 'column':
                        if self.sco.z_tar_1 > self.sco.z_tar_2:
                            ...
                            if self.sco.collision_e == True:
                                ...
                                if self.sco.z_1 <= self.s_he - self.sco.length + 1 and (
                                        self.site_3D[self.sco.z_2 - 1][self.sco.x_2][self.sco.y_2] == 1 or
                                        self.site_3D[self.sco.z_2 - 1][self.sco.x_2][
                                            self.sco.y_2] == 10):  # if component on ground or has a other component under
                            ...
                            if self.sco.collision_e == True:
                                ...
                                if self.sco.z_2 <= self.s_he - self.sco.length + 1 and (
                                        self.site_3D[self.sco.z_1 - 1][self.sco.x_1][self.sco.y_1] == 1 or
                                        self.site_3D[self.sco.z_1 - 1][self.sco.x_1][self.sco.y_1] == 10):
                                    if self.sco.z_1 == self.sco.z_2:
                    ...
                    if self.sco.collision_lay == True:
                        ...
                        if self.sco.z_1 != self.sco.z_2:
                            if self.sco.z_1 < self.sco.z_2:
                                ...
                                z = self.sco.z_2
                            if self.site_3D[z - 1][self.sco.x_2][self.sco.y_2] == 1 or \
                                    self.site_3D[z - 1][self.sco.x_2][self.sco.y_2] == 10:
                            ...
                            if self.sco.y_1 - self.sco.length + 2 > 0:
                                ...
                                for x_len in range(self.sco.length):
                                    for y_len in range(self.sco.length - 1):
                                        if self.site_3D[self.sco.z_1][x1 + x_len][self.sco.y_1 - y_len - 1] == 1 or \
                                                self.site_3D[self.sco.z_1][x1 + x_len][self.sco.y_1 - y_len - 1] == 2:
                            ...
                            if self.sco.x_1 + self.sco.length - 1 < self.s_wid:
                                ...
                                for y_len in range(self.sco.length):
                                    for x_len in range(self.sco.length - 1):
                                        if self.site_3D[self.sco.z_1][self.sco.x_1 + x_len + 1][y1 + y_len] == 1 or \
                                                self.site_3D[self.sco.z_1][self.sco.x_1 + x_len + 1][y1 + y_len] == 2:
                                            self.sco.collision_rotate1 = True
                                            self.site_3D[self.sco.z_1][self.sco.x_1][self.sco.y_1] = 1
                                            self.site_3D[self.sco.z_2][self.sco.x_2][self.sco.y_2] = 1
                                            self.move_sco(self.sco)
